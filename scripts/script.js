...
const snsS = new Swiper('.sns .swiper',{
    slidesPerView:2, //한번에 보일 슬라이드 수(기본 모바일)
    spaceBetween:20, //슬라이드 사이 여백
    /* 움직임 작성 */
    /* autoplay:{delay:1000,}, */ /* 딜레이주기 */
...
for(let detail of detail_img){ // 6개 이미지 반복문 접근
    detail.addEventListener('click',(e)=>{
        ...
        e.preventDefault() // (e) 자동으로 올라가는거 방지
        // 큰 이미지 부모 보이기
        big_bg.style.display = 'block'
        // 팝업 실행 시 body 스크롤 막기
        document.body.style.overflow = 'hidden'; // 밖에 스크롤 움직임 방지
        console.log(detail.src)
        // 클릭한 이미지의 경로(src)를 큰 이미지 팝업의 src로 대입하기
        big_img.src = detail.src // 이미지 주소 팝업에 대입
        // 이전 팝업에서 내릴 스크롤을 기억하지 않도록 항상 스크롤 위로 올리기
        big_bg.children[0].scrollTo(0,0) // 팝업 스크롤 초기화(새로운 팝업 띄울때 항상 이미지 처음에서 시작하기)
    })
}
...
big_bg.addEventListener('click',()=>{
    ...
    document.body.style.overflow = ''; // 밖에 스크롤 다시 움직이게 하기
})